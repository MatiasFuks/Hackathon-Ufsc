...
| **Horizonte** | **quando** atacar: Release (0–14d), Furacão (15–30d), 4–8 semanas ou Backlog. É um eixo **separado** da prioridade (ver nota abaixo). |
| **Release × Furacão** | a HourTrack tem **dois** prazos: a release em 14 dias (congelamento) e a auditoria do contrato em 30. Por isso a janela de urgência é dividida em duas. |
...
A HourTrack tem **dois prazos distintos**: a **release em 14 dias** (uma janela
de *congelamento* — nada de refatoração arriscada, sem ambiente de teste) e a
**auditoria do contrato em 30 dias**. Por isso a antiga janela única de urgência
foi dividida em duas: só os **bloqueadores de contrato** entram na release; o
resto da segurança crítica corre **depois** dela, ainda antes da auditoria.

| Horizonte | Entra aqui quem… |
|-----------|------------------|
| 🚨 **Release (0–14 dias)** | é **bloqueador de contrato** (Q1/Q2 + confiança Alta) — trava a assinatura, começa no dia 1 |
| 🌀 **Furacão (15–30 dias)** | é Crítica que **não** trava contrato e é barata (≤ 5 SP) — segurança restante, pós-release e pré-auditoria |
...
> Por que os bloqueadores grandes não "estouram" a release? O multiplicador
> **× 0.5** (seção 2) já derruba o esforço > 8 SP **antes** daqui — então um
> refactor gigante nunca chega a competir pela janela de 14 dias.
...
|---|---------|------|-----------|------------------|-----------|---------|------:|----------------|---------------|
| 1 | SQLi no login (exposto) | Alta | Segurança | público + enterprise, **Q1**, 3 SP | Alta | 70 × 2.0 × 1.5 × 1.0 | **210** | Crítica | 🚨 Release (0–14d) *(bloqueador Q1)* |
| 2 | XSS refletido | Alta | Segurança | público + enterprise, **Q2**, 2 SP | Alta | 70 × 2.0 × 1.5 × 1.0 | **210** | Crítica | 🚨 Release (0–14d) *(bloqueador Q2)* |
| 3 | `debug=True` em produção | Crítica | Segurança | Q6, 1 SP | Alta | 100 × 2.0 × 1.0 | **200** | Crítica | 🌀 Furacão (15–30d) *(Crítica ≤ 5 SP, não bloqueia)* |
| 4 | Credencial hardcoded | Crítica | Segurança | Q4, 2 SP | Média | 100 × 2.0 × 0.8 | **160** | Crítica | 🌀 Furacão (15–30d) *(Crítica ≤ 5 SP, não bloqueia)* |
| 5 | Refatorar auth vulnerável | Crítica | Segurança | Q6, 7 SP | Alta | 100 × 2.0 × 1.0 | **200** | Crítica | 🔧 4–8 semanas *(> 5 SP, cara pra janela)* |
...
- **#1 vs #7** — o *mesmo* SQLi. Com confiança Alta é bloqueador: Crítica + **Release (0–14d)**
  (override). Com confiança Baixa cai pra Alta/4–8 semanas e **não** dispara o
  override — a defesa anti-falso-positivo em ação. É essa régua de confiança que
  separa o que entra na release do que espera o Furacão.
- **#3** — chega a Crítica **sem** override (o score já basta): o override não é
  o único caminho pra Crítica. Como não é bloqueador, vai pro Furacão (15–30d), não pra release.
- **#4 vs #5** — duas Críticas de segurança não-bloqueadoras; o esforço decide o horizonte
  (2 SP → Furacão 15–30d; 7 SP → 4–8 semanas, porque não cabe na janela).
...
horizonte: é bloqueador de contrato      → 🚨 Release (0–14 dias)
```
